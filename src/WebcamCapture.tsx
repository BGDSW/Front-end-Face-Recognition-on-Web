  ...
  return (
    <>
      {isCapturing ? (
        ...
        <img className='Placeholder' src={imageSrc || placeholderImage} alt="Placeholder" />
      )}
      {isCapturing ? (
        <button className='CamButton1' onClick={stopCam}>Stop Webcam</button>
      ) : (
        <button className='CamButton2' onClick={startCam}>Start Webcam</button>
      )}
      {isCapturing && <button className='CapButton' onClick={capture}>Capture</button>}
      {<img className='ProcessedImage' src={processedImageSrc} alt="processedImage"/>}
      <canvas className='Canvas' ref={canvasRef} width={width} height={height}></canvas>
      <input className='UploadImg' type="file" accept="image/*" onChange={handleFileInputChange} />
    </>
  );
};
